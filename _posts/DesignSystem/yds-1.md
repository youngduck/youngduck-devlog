...
**의문점**: 공식문서에 따르면 inject는 환경과 무관하게 동작해야 하는데, 왜 로컬에서는 번들자체만으로 디자인이 적용되고 빌드에서는 적용이 안 되는지 정확한 원인을 파악하지 못했습니다. 추후 번들링 결과물 자체를 분석해봐야할 것 같습니다.
...
이렇게 처리를 하니 로컬 개발 환경과 프로덕션 빌드 모두에서 YD-UI 스타일이 적용되는것을 드디어 확인 할 수 있었습니다.

&nbsp;

## 문제를 해결하며 궁금했던 부분들

문제를 해결하기 위해서 라이브러리의 소스코드를 고치고 보돌코 프로젝트에서 테스트를 하는 과정에서 라이브러리를 배포해야만 프로젝트에서 테스트를 할 수 있다보니 yd-ui라이브러리의 버전이 **무진장** 치솟는 이슈가 있었습니다.
실무에서는 디자인시스템팀의 작업물을 어떻게 라이브하게 테스트 할까라는 궁금증이 생겼으나 당장은 해당 이슈를 고치는데 집중하였고 다음 포스팅에 이러한 내용을 적어볼까 합니다.
서칭을 해보니 npm link를 활용해서 개발환경을 구성할 수 있는 것으로 보이네요.
